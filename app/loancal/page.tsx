...
export default function LoanCalPage() {
  return (
    <main className="relative min-h-screen flex flex-col justify-between overflow-x-hidden bg-gradient-to-br from-[#f2fcf7] via-[#e6f9f0] to-[#daf5e8] text-slate-800 antialiased">
      {/* Decorative Ambient Background Glows (Mint Green Theme) */}
      <div className="pointer-events-none fixed inset-0 overflow-hidden" aria-hidden="true">
        <div className="absolute -top-32 -left-20 h-96 w-96 rounded-full bg-emerald-200/35 blur-3xl" />
        <div className="absolute top-1/4 -right-20 h-96 w-96 rounded-full bg-teal-200/40 blur-3xl" />
        <div className="absolute -bottom-24 left-1/4 h-80 w-80 rounded-full bg-emerald-300/25 blur-3xl" />
        <div className="absolute top-2/3 right-1/4 h-72 w-72 rounded-full bg-teal-300/25 blur-3xl" />
      ...
      <header className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 flex items-center justify-between">
        <Link
          href="/"
          className="group inline-flex items-center gap-2.5 text-sm font-semibold text-emerald-900 hover:text-emerald-950 px-4 py-2.5 rounded-2xl bg-white/80 hover:bg-white border border-emerald-200/80 shadow-xs hover:shadow-md transition-all duration-200"
        >
          <svg
            className="w-4 h-4 transition-transform duration-200 group-hover:-translate-x-1 text-emerald-600"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
            strokeWidth="2.5"
        ...
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-lg bg-gradient-to-tr from-emerald-600 via-teal-500 to-emerald-400 p-0.5 shadow-xs">
      ...
      {/* Footer */}
      <footer className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 border-t border-emerald-200/70 text-center text-xs text-slate-500 flex flex-col sm:flex-row items-center justify-between gap-3">
